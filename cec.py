...
class CEC(object):
    ...
    def convert(self, options, args):
        ...
        for file in files:
            ...
            df = pd.read_csv(file, delimiter=options.input_delimiter, header='infer', encoding=options.input_encoding)
            filename, ext = splitext(basename(file))
            df.to_csv("result/{0}_{1}{2}".format(filename, options.output_encoding, ext), mode='w', sep=options.output_delimiter, index=False, header=True, encoding=options.output_encoding)
